...
let formValidation = function(e) {
    ...
    if (result == false) {
        document.getElementById('form-box__area__headline').innerText = "Введите корректные данные"
    }
    if (result == true) {
...
let mailValidation = (mail) => {
    ...
    if (mail.value.match(regexp)) { 
        ...
        spanErrorText[1].innerText = 'Адрес эл. почты может содежрать латинские буквы и (@, . - _)';
